...
onready var Asteroid_small = load("res://Asterorid_small.tscn")
	...
	if health <= 0:
		...
		if Asteroid_Container != null: 
			for s in small_asteroids: 
				...
				Asteroid_Container.call_defer("add_child", asteroid_small)
				asteroid_small.position = position + s.roatated(dir)
